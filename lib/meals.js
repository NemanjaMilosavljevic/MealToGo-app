...
export const filterMealsByQueryParams = (vegge, fasting, maxPrice, ...arg) => {
  if (arg.length === 0) {
    if (!vegge && fasting) {
      ...
      return db
        .prepare(
          "SELECT * FROM meals WHERE vegan = 1 AND fasting = 1 AND (price <= ?)"
        )
        .all(maxPrice);
    } else {
    ...
    if (!vegge && fasting) {
      ...
      return db
        .prepare(
          "SELECT * FROM meals WHERE vegan = 1 AND fasting = 1 AND (price <= ?) AND category = ?"
        )
        .all(maxPrice, arg[0]);
    } else {
      return db
    ...
    if (!vegge && fasting) {
      ...
      return db
        .prepare(
          "SELECT * FROM meals WHERE vegan = 1 AND fasting = 1 AND (price <= ?) AND subcategory = ?"
        )
        .all(maxPrice, arg[0]);
    } else {
      return db
